...
module.exports = async function cleanupTempFiles() {
  ...
  try {
    ...
    for (const filename of files) {
      ...
      try {
        ...
        if (fileAge > ONE_HOUR) {
          ...
          cleanedCount++;
          logger.info({ filePath, ageHours: (fileAge / ONE_HOUR).toFixed(2) }, "[cleanup.tempfiles] deleted stale file");
        }
      } catch (err) {
        errorCount++;
    ...
    if (cleanedCount > 0 || errorCount > 0) {
      logger.info({ cleanedCount, errorCount, totalFiles: files.length }, "[cleanup.tempfiles] completed");
    }
  } catch (err) {
