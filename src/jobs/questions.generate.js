...
module.exports = async function questionsGenerate(payload) {
  ...
  try {
    const result = await client.generateQuestions({ contextText, numQuestions, difficulty, topics: [] });
    if (Array.isArray(result.questions) && result.questions.length) {
      await qrepo.updateById(questionSetId, { $set: { questions: result.questions } }, { new: true });
    }
  } catch (e) {
