...
const requestRevisionSchema = Joi.object({
  ...
  body: Joi.object({
    learnerResponse: Joi.string().required().min(10).max(1000),
  }),
}).unknown(true);
...
router.patch('/:id/request-revision', rateLimit({ limit: 30 }), authenticateJWT, authorizeRole('Learner'), inputValidation(requestRevisionSchema), controller.requestRevision);
